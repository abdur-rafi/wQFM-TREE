    ...
    for dup in $(ls $root/$k); do 
        for loss in $(ls $root/$k/$dup); do
            for ils in $(ls $root/$k/$dup/$loss); do
                ...
                echo "$k-$dup-$loss-$ils-100bp " >> 25.wqfm.2020.norm.scores
                python scripts/scoreSep.py < $root/$k/$dup/$loss/$ils/e100_wqfm_2020_norm_scores.txt >> 25.wqfm.2020.norm.scores
                # exit
            done
        done
    done
